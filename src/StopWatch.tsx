...
export default function Stopwatch(){
  ...
  return (
    <div>
      ...
      <StopWatchButton disabled={running} onClick={startStopwatch} text="Start" />
      <StopWatchButton disabled={!running} onClick={stopStopwatch} text="Stop" />
      <StopWatchButton disabled={!running && stoppedLaps} onClick={lapStopwatch} text="Lap" />
      <StopWatchButton disabled={running} onClick={resetStopwatch} text="Reset" />
      {laps.length > 0 && (
        <ul>
          {laps.map((lap, index) => (
